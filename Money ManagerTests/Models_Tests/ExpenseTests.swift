import Foundation
import SwiftData
import Testing
@testable import Money_Manager

struct ExpenseModelTests {
    
    @Test
    func testExpenseIsRecurringWhenFlagSet() {
        let expense = Expense(
            amount: 500,
            category: "Food & Dining",
            date: Date(),
            isRecurring: true,
            frequency: "monthly",
            dayOfMonth: 15
        )
        
        #expect(expense.isRecurring == true)
        #expect(expense.frequency == "monthly")
        #expect(expense.dayOfMonth == 15)
    }
    
    @Test
    func testExpenseIsNotRecurringByDefault() {
        let expense = Expense(
            amount: 500,
            category: "Food & Dining",
            date: Date()
        )
        
        #expect(expense.isRecurring == false)
        #expect(expense.frequency == nil)
        #expect(expense.dayOfMonth == nil)
    }
    
    @Test
    func testExpenseDefaultValuesOnInit() {
        let expense = Expense(
            amount: 100,
            category: "Transport",
            date: Date()
        )
        
        #expect(expense.isDeleted == false)
        #expect(expense.isActive == true)
        #expect(expense.time == nil)
        #expect(expense.expenseDescription == nil)
        #expect(expense.notes == nil)
        #expect(expense.groupId == nil)
        #expect(expense.groupName == nil)
        #expect(expense.recurringEndDate == nil)
    }
    
    @Test
    func testExpenseWithGroupData() {
        let groupId = UUID()
        let expense = Expense(
            amount: 1000,
            category: "Travel",
            date: Date(),
            groupId: groupId,
            groupName: "Goa Trip"
        )
        
        #expect(expense.groupId == groupId)
        #expect(expense.groupName == "Goa Trip")
    }
    
    @Test
    func testExpenseWithAllOptionalParameters() {
        let expenseTime = Date()
        let groupId = UUID()
        let endDate = Calendar.current.date(byAdding: .year, value: 1, to: Date())!
        
        let expense = Expense(
            amount: 250,
            category: "Shopping",
            date: Date(),
            time: expenseTime,
            expenseDescription: "New shirt",
            notes: "Gift for friend",
            isRecurring: true,
            frequency: "monthly",
            dayOfMonth: 1,
            recurringEndDate: endDate,
            groupId: groupId,
            groupName: "Office Lunch"
        )
        
        #expect(expense.time == expenseTime)
        #expect(expense.expenseDescription == "New shirt")
        #expect(expense.notes == "Gift for friend")
        #expect(expense.isRecurring == true)
        #expect(expense.frequency == "monthly")
        #expect(expense.recurringEndDate == endDate)
        #expect(expense.groupName == "Office Lunch")
    }
    
    @Test
    func testExpenseAmountCanBeZero() {
        let expense = Expense(
            amount: 0,
            category: "Other",
            date: Date()
        )
        
        #expect(expense.amount == 0)
    }
    
    @Test
    func testExpenseAmountCanBeNegative() {
        let expense = Expense(
            amount: -50,
            category: "Other",
            date: Date(),
            notes: "Refund"
        )
        
        #expect(expense.amount == -50)
    }
    
    @Test
    func testRecurringExpenseWithEndDate() {
        let startDate = Date()
        let endDate = Calendar.current.date(byAdding: .year, value: 1, to: startDate)!
        
        let expense = Expense(
            amount: 649,
            category: "Entertainment",
            date: startDate,
            expenseDescription: "Netflix",
            isRecurring: true,
            frequency: "monthly",
            dayOfMonth: 1,
            recurringEndDate: endDate
        )
        
        #expect(expense.recurringEndDate == endDate)
        #expect(expense.isActive == true)
    }
    
    @Test
    func testRecurringExpenseDeactivation() {
        let expense = Expense(
            amount: 649,
            category: "Entertainment",
            date: Date(),
            expenseDescription: "Netflix",
            isRecurring: true,
            frequency: "monthly"
        )
        
        #expect(expense.isActive == true)
        expense.isActive = false
        #expect(expense.isActive == false)
    }
    
    @Test
    func testRecurringExpenseSupportsAllFrequencies() {
        let frequencies = ["daily", "weekly", "monthly", "yearly"]
        
        for frequency in frequencies {
            let expense = Expense(
                amount: 100,
                category: "Other",
                date: Date(),
                isRecurring: true,
                frequency: frequency
            )
            #expect(expense.frequency == frequency)
        }
    }
    
    @Test
    func testGroupExpenseCanBeRecurring() {
        let groupId = UUID()
        let expense = Expense(
            amount: 1500,
            category: "Utilities",
            date: Date(),
            expenseDescription: "Monthly Rent Split",
            isRecurring: true,
            frequency: "monthly",
            dayOfMonth: 1,
            groupId: groupId,
            groupName: "Roommates"
        )
        
        #expect(expense.isRecurring == true)
        #expect(expense.groupId == groupId)
        #expect(expense.frequency == "monthly")
    }
}
